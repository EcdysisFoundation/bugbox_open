...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for start in range(0, total, batch_size):
            ...
            for specimen_image in batch:
                try:
                    ...
                    if (specimen_image.image_thumbnail_large and
                        ...
                        if default_storage.exists(specimen_image.image_thumbnail_large.name):
                            try:
                                ...
                                self.stdout.write(
                                    self.style.WARNING(
                                        f'Could not read dimensions for thumbnail {
                                            specimen_image.image_thumbnail_large.name}: {e}'))
                    ...
                    if processed % 500 == 0:
                        self.stdout.write(
                            f'Processed {processed}/{total} records ({updated} updated, {errors} errors)...')
                        gc.collect()
        ...
        self.stdout.write(self.style.SUCCESS(f'Completed: processed {processed} records, updated {
            updated} records with dimensions, {errors} errors.'))
