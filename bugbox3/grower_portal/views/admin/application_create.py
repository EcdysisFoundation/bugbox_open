...
from ...forms.admin.application_forms import (
    GrowerSelectionForm,
    AdminApplicationCreationForm
)
...
def admin_application_create_step1(request, application_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            try:
                ...
                if 'field_id_date_sampled' in str(e):
                    messages.error(
                        request,
                        f'An application already exists for this field on {form.cleaned_data["date_sampled"].strftime("%B %d, %Y")}.'
                    )
                else:
        ...
        initial_data = {
            ...
            'crop_type': application.field.crop_type if application.field and application.field.crop_type else '',
            'crop_subtype': application.field.crop_subtype if application.field and application.field.crop_subtype else '',
            'crop_subtype_other': application.field.crop_subtype_other if application.field and application.field.crop_subtype_other else '',
            'small_grain_type': application.field.small_grain_type if application.field and application.field.small_grain_type else '',
            'tillage_methods': application.field.tillage_methods if application.field and application.field.tillage_methods else '',
            'forage_varieties': application.field.forage_varieties if application.field and application.field.forage_varieties else '',
            'paddock_size': application.field.paddock_size if application.field and application.field.paddock_size else '',
            'pasture_size': application.field.pasture_size if application.field and application.field.pasture_size else '',
            'rootstock_species': application.field.rootstock_species if application.field and application.field.rootstock_species else '',
            'crop_varieties': application.field.crop_varieties if application.field and application.field.crop_varieties else '',
            'transitional_status': application.field.transitional_status if application.field and application.field.transitional_status else '',
            'orchard_crop_type': application.field.crop_type if application.field and application.field.field_type == 'orchard' else '',
            'orchard_crop_specify': application.field.orchard_crop_specify if application.field and application.field.orchard_crop_specify else ''
        }
