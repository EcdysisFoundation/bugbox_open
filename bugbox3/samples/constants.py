...
FIELD_MULTIIMAGE_PANORAMA_FILENAME = 'panorama_filename'
FIELD_MULTIIMAGE_UPLOAD_DIR_NAME = 'upload_dir_name'
FIELD_MULTIIMAGE_ANNOTATIONS = 'annotations'
FIELD_MULTIIMAGE_ANNOTATIONS_UPDATED_AT = 'annotations_updated_at'
FIELD_MULTIIMAGE_PREDICTIONS = 'predictions'
FIELD_MULTIIMAGE_PREDICTIONS_TIMESTAMP = 'predictions_timestamp'
FIELD_MULTIIMAGE_IMAGE = 'image'
FIELD_MULTIIMAGE_IMAGE_THUMBNAIL = 'image_thumbnail'
FIELD_MULTIIMAGE_LABEL_IMAGE = 'label_image'
FIELD_MULTIIMAGE_LABEL_IMAGE_THUMBNAIL = 'label_image_thumbnail'
...
FIELD_SPECIMEN_AI_MODEL_NAME = 'ai_model_name'
...
FIELD_SPECIMEN_OMIT_FROM_TRAINING = 'omit_from_training'
...
STITCHER_SAMPLE_TYPES = {
    'qu': SAMPLE_TYPE_QUADRAT,
    'sw': SAMPLE_TYPE_VEGETATION_SWEEP
}
...
ANNOTATIONS_CHOICE_COCO = 'coco'
ANNOTATIONS_CHOICE_YOLO = 'yolo'
ANNOTATIONS_FORMAT_CHOICES = (
    (ANNOTATIONS_CHOICE_COCO, 'COCO'),
    (ANNOTATIONS_CHOICE_YOLO, 'YOLO')
)
...
INDICES_CHOICES_ALL = (
    INDICES_ALWAYS_INCLUDED +
    [v[0] for v in INDICES_CHOICES] +
    ['hill_H0', 'hill_H1', 'hill_H2', 'hill_inf']
)
...
INDICES_DESCRIPTIONS = {
    'hill_numbers': (
        "Hill numbers are a family of diversity indices that combine species richness and evenness. "
        "They include measures like richness, exponential Shannon, and inverse Simpson, providing various "
        "ways to express community diversity."
    ),
    'shannons_h': (
        "Shannon’s H quantifies the uncertainty in predicting the species identity of an individual randomly "
        "chosen from the sample, accounting for both species richness and evenness in the distribution of individuals."
    ),
    'simpsons_d': (
        "Simpson’s D measures the probability that two randomly selected individuals from a sample belong to the "
        "same species, with a lower value indicating higher diversity."
    ),
    'simpsons_d_prime': (
        "Simpson’s D’ is a bias-corrected version of Simpson’s D, providing a more reliable estimate of diversity "
        "by adjusting for biases in smaller samples. (Lower value means higher diversity)."
    ),
    'gini_simpson': (
        "Gini-Simpson’s 1-D index is derived from the Gini coefficient "
        "and measures the probability that two individuals "
        "from a community will belong to different species, with higher values indicating higher diversity."
    ),
    'chao1': (
        "Chao1 is an estimator for species richness that accounts for undetected species, providing a more accurate "
        "estimate of true species richness, particularly in small or incomplete samples."
    ),
    'absolute_effective_diversity': (
        "Absolute Effective Diversity (AED) adjusts for unequal sampling efforts "
        "and species abundance, emphasizing the "
        "actual diversity in an ecosystem without overemphasizing rare or dominant species."
    ),
    'abundance': (
        "Abundance is the total count of individuals across all species in a community,"
        " giving a sense of how populous "
        "the ecosystem is, regardless of species types."
    ),
    'species_richness': (
        "Species richness simply counts how many distinct species are present in a community, giving an idea of the "
        "biodiversity, independent of how many individuals each species has."
    )
}
...
FORM_FIELDS_SITE = (
    ...
    FIELD_SITE_LATITUDE,
    FIELD_SITE_STATE_REGION,
    FIELD_SITE_COUNTY_REGION
)
...
FORM_FIELDS_SPECIMEN = (
    ...
    FIELD_SPECIMEN_OMIT_FROM_TRAINING
)
...
FORM_FIELDS_SPECIMEN_LABELS = {
    ...
    FIELD_SPECIMEN_OMIT_FROM_TRAINING: 'Omit from model training'
}
...
EXP_HEAD_ARR_HABITAT = 'Habitat'
EXP_HEAD_ARR_TREATMENT = 'Treatment'
...
EXP_HEADERS_ARR = [
    ...
    EXP_HEAD_ARR_SITE,
    EXP_HEAD_ARR_HABITAT,
    EXP_HEAD_ARR_TREATMENT,
    EXP_HEAD_ARR_DATE,
...
# Serializer Fields
SERIALIZER_FIELDS_MUTIIMAGE = [
    FIELD_MULTIIMAGE_UUID,
    FIELD_MULTIIMAGE_PANORAMA_FILENAME,
    FIELD_MULTIIMAGE_UPLOAD_DIR_NAME,
    FIELD_MULTIIMAGE_ANNOTATIONS,
    FIELD_MULTIIMAGE_ANNOTATIONS_UPDATED_AT,
    FIELD_MULTIIMAGE_PREDICTIONS,
    FIELD_MULTIIMAGE_PREDICTIONS_TIMESTAMP,
    FIELD_MULTIIMAGE_IMAGE
]
